...
#     catchup=False  # Schedule to run every Sunday at 01:00 AM
# )
...
def runCode(): 
    ...
    try:
        if client.get_table(premium_prod_table).exists():
        ...
        pp_table = bigquery.Table(premium_prod_table, schema=premium_prod_job_config.schema)
        pp_table = client.create_table(pp_table)
        premium_prod_job = client.load_table_from_dataframe(new_df, premium_prod_table, job_config=premium_prod_job_config)
        premium_prod_job.result()
    ...
    try:
        if client.get_table(price_optimize_table).exists():
            ...
            price_optimize_job_config.result()
    except:
        po_table = bigquery.Table(price_optimize_table, schema=price_optimize_schema)
        po_table = client.create_table(po_table)
        price_optimize_job = client.load_table_from_dataframe(new_df2, price_optimize_table, job_config=price_optimize_job_config)
        price_optimize_job_config.result()
    ...
    try:
        if client.get_table(table_ref).exists():
            # If the table exists, load data into it
        ...
        # If the table doesn't exist, create it and load data into it
        ir_table = bigquery.Table(sales_summary_table, schema=sales_summary_schema)
        ir_table = client.create_table(ir_table)
        sales_summary_job = client.load_table_from_dataframe(sales_summary, sales_summary_table, job_config=sales_summary_job_config)
        sales_summary_job.result()

    # 3rd Table

# Define the operator that runs the Python function
# run_my_python_code = DataFlowPythonOperator(
#     task_id='etl_task',
#     python_callable=runCode,
#     dag=dag
# )

with models.DAG('ecommerce',
                default_args=default_args,
                schedule_interval='@weekly',
                catchup=False) as dag:
    t1 = DataFlowPythonOperator(
        task_id='etl_task',
        py_file='gs://us-east1-ecommerce-env-ca9d7c43-bucket/test3.py',
        # python_callable=runCode,
    )